...
#if defined(NDEBUG)
# error "Fireants cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "Fireants Signed Message:\n";
...
CAmount GetProofOfWorkSubsidy()
{
    ...
    if (nBlockHeight >=       1 && nBlockHeight <=  600000 ) { return 100 * COIN; }
    if (nBlockHeight >=  600001 && nBlockHeight <= 1000000 ) { return  40 * COIN; }
    if (nBlockHeight >= 1000001 && nBlockHeight <= 1500000 ) { return  20 * COIN; }
    if (nBlockHeight >= 1500001 && nBlockHeight <= 2000000 ) { return  10 * COIN; }
    if (nBlockHeight >= 2000001 ) { return   5 * COIN; }
}
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    // Fireants also requires DER encoding of pubkeys
    flags |= SCRIPT_VERIFY_DERKEY;
    // Fireants also requires low S in sigs
